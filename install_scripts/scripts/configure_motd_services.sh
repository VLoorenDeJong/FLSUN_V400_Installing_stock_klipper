...
# addresses, pending-updates count, ESM status — in place of a stripped-down
# or custom-branded banner left over from FLSUN's stock image.
# =============================================================================
...
if [ ${#NEEDED_PKGS[@]} -gt 0 ]; then
    ...
    show_progress "📦 Installing ${NEEDED_PKGS[*]}" \
        "DEBIAN_FRONTEND=noninteractive apt-get install -y -qq ${NEEDED_PKGS[*]} >/dev/null 2>&1" 3 300
...
# FLSUN's stock image is known to inject a custom credit line (e.g. "By
# <name> Running on ...") in place of the standard dynamic blocks. Kept as an
# array (not a single regex) so a different vendor's custom-banner style
# encountered on another Pi project can be added as one extra line here,
# rather than restructuring this section. Scans /etc/motd and every
# update-motd.d script; disables (never deletes) anything found — renamed
# with a .disabled-by-configure-motd-services suffix so it stays fully reversible.
...
# Editing /etc/pam.d/sshd automatically is too risky on a device with no
# physical console — a malformed PAM config can break SSH login entirely,
# the same "lost is lost" consequence as the WiFi ground rule in CLAUDE.md.
# Detect and report only; never auto-edit this file.
